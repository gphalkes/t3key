  ...
  for (current = keys; current != NULL; current = current->next) {
    if (current->duplicate != NULL) {
      if (!option_no_duplicates) {
        fprintf(output, "\t\t# %s%s = %s%s\n", current->keynames->identifier,
                current->modifiers->identifier, current->duplicate->keynames->identifier,
                current->duplicate->modifiers->identifier);
      }
    } else {
      fprintf(output, "\t\t%s%s = \"%s\"\n", current->keynames->identifier,
              current->modifiers->identifier, current->seq);
    }
  }
}
...
static void extract_shared_maps(map_t *head, map_t *last_new) {
  ...
  for (current_map = head; current_map != NULL; current_map = current_map->next) {
    ...
    for (search_for = last_new->sequences; search_for != NULL; search_for = search_for->next) {
      for (current_key = current_map->sequences; current_key != NULL;
  ...
  OPTIONS
    ...
    OPTION('h', "help", NO_ARG)
      printf("Usage: t3learnkeys [<options>]\n"
        ...
        "  -s,--show-only              Only show the sequences generated by pressing keys\n"
      );
      exit(EXIT_SUCCESS);
    END_OPTION
    OPTION('s', "show-only", NO_ARG)
      option_free_mode = true;
    END_OPTION
    DOUBLE_DASH
      NO_MORE_OPTIONS;
    END_OPTION
...
int main(int argc, char *argv[]) {
  ...
  if (option_free_mode) {
    ...
    while (1) {
      do {
        printf(
            "Escape sequence or name of terminfo entry to enter desired mode (- for current "
            "mode): ");
        buffer[0] = 0;
        scanf("%1023s", buffer);
      } while (buffer[0] == 0);
      if (buffer[0] == '\\') {
        start = parse_escapes(buffer);
        break;
    ...
    if (start != NULL) {
      printf(
          "After ending the program, you may wish to run the reset program to ensure that all keys "
  ...
  if (option_auto_learn) {
    printf(
        "Automatically learning keys for terminal %s. DO NOT press a key while the key learning is "
        "in progress.\n",
        term);
    maxfkeys = 35;
  } else {
  ...
  for (mode_ptr = mode_head; mode_ptr != NULL && mode_ptr->name != NULL;
  ...
  fprintf(output,
          "# If the terminal supports XTerm style mouse reporting, uncomment the "
          "following:\n#xterm_mouse = true\n\n");

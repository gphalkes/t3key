...
int main(int argc, char *argv[]) {
	...
	if (option_free_mode) {
		...
		while (1) {
			do {
				printf("Escape sequence or name of terminfo entry to enter desired mode (- for current mode): ");
				buffer[0] = 0;
				scanf("%1023s", buffer);
			} while (buffer[0] == 0);
			if (buffer[0] == '\\') {
		...
		init_terminal();
		if (start != NULL) {
			printf("After ending the program, you may wish to run the reset program to ensure that all keys are working as expected.\n");
			putp(start);
			fflush(stdout);
		}

...
static void add_sequence(t3_config_t *ptr, t3_config_t *noticheck) {
	...
	if (i == ARRAY_LENGTH(valid_names)) {
		if (!(name[0] == 'f' && is_asciidigit(name[1]) &&
				(name[2] == 0 || name[2] == '-' || (is_asciidigit(name[2]) &&
				(name[3] == 0 || name[3] == '-') && name[1] != '0'))))
		{
	...
	if (option_check_terminfo && !invalid_name && t3_config_find(noticheck, compare_name, name, NULL) == NULL) {
		...
		//FIXME: get file name information
				fprintf(stderr, "%s:%d: '%s' has different definition (%s) than terminfo (%s)\n",
					input, t3_config_get_line_number(ptr), t3_config_get_name(ptr),
					t3_config_get_string(ptr), get_print_seq(tistr));
			}
		}
...
static void create_symlinks(t3_config_t *map_config, const char *term_name) {
	...
	for (aka = t3_config_get(t3_config_get(map_config, "aka"), NULL); aka != NULL; aka = t3_config_get_next(aka)) {
